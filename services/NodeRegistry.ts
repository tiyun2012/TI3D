...
export const NodeRegistry: Record<string, NodeDef> = {
  ...
  'SimplexNoise': {
      ...
      inputs: [
          ...
          { id: 'time', name: 'Time', type: 'float', optional: true }
      ],
      outputs: [{ id: 'out', name: 'Value (-1..1)', type: 'float' }],
      execute: () => 0, 
      glsl: (inVars, id) => {
          const funcName = `snoise_${id}`;
          return {
              functions: `
                // Simplex 3D Noise 
              ...
              `,
              body: `
                vec3 ${id}_pos = ${inVars[0] || 'vec3(0.0)'};
                float ${id}_scale = ${inVars[1] || '1.0'};
                float ${id}_time = ${inVars[2] || '0.0'};
                float ${id} = ${funcName}((${id}_pos * ${id}_scale) + vec3(0.0, ${id}_time, 0.0));
              `
          };
      }
  },
